...
def calculation(depth):
    danger_level = depth ** 3 - 3 * depth ** 2 - 12 * depth + 10
    return danger_level
def middle(depth_min,depth_max):
    depth = depth_min+(depth_max - depth_min) / 2
    return depth
danger_level = 0
depth_max = 4
depth_min = 0
maximum_danger_level = float(input('ведите максимально допустимый уровень опасности: '))
danger_level = calculation(middle(depth_min,depth_max))
depth = middle(depth_min,depth_max)
while abs(danger_level) > maximum_danger_level:
    if danger_level < 0:
        depth_max = depth
        depth = middle(depth_min,depth_max)
        danger_level = calculation(depth)
    else:
        depth_min = depth
        depth = middle(depth_min, depth_max)
        danger_level = calculation(depth)
